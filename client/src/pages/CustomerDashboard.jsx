...
import { Search, Calendar, Dumbbell, Sparkles, MapPin } from "lucide-react";
import { SPORTS_LIST, getSport } from "../constants/sports";
import { INIT_CLUBS, SLOT_MAP, DAYS, DATES, MONTHS, MOCK_BOOKINGS, RENTAL_ORDERS, EQUIPMENT_CATALOG } from "../constants/mockData";
...
export default function CustomerDashboard() {
  ...
  return (
    <div>
      ...
      {tab === "discover" && (
        <div>
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
            <button onClick={() => setActiveSport(null)} className={`px-4 py-2 rounded-full font-semibold text-sm whitespace-nowrap transition-all ${activeSport === null ? "bg-[#08060d] text-white" : "bg-white text-gray-500 shadow-sm"}`}>
              All Sports
            </button>
            {SPORTS_LIST.map((sport) => {
              const isActive = activeSport === sport.id;
              return (
                <button key={sport.id} onClick={() => setActiveSport(isActive ? null : sport.id)}
                  className="px-4 py-2 rounded-full font-semibold text-sm whitespace-nowrap flex items-center gap-1.5 transition-all"
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-gray-400">No clubs found for this filter.</div>
          ) : (
      ...
      {tab === "recommended" && <RecommendedTab clubs={clubs} onSelect={(c) => { setBookingClub(c); setSelectedSlot(null); setSelectedCourt(c.courts?.[0]); }} />}
          ...
          <div>
            <div className="mb-5">
              ...
              <p className="text-sm text-gray-500 mt-1 flex items-center gap-1">
                <MapPin size={13} /> {bookingClub.location} · ₹{bookingClub.price}/hr
              </p>
                ...
                {bookingClub.courts?.map((court) => {
                  ...
                  return (
                    <button key={court.id} onClick={() => setSelectedCourt(court)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${selectedCourt?.id === court.id ? "text-white" : ""}`}
                      style={selectedCourt?.id === court.id ? { background: sport?.color } : { background: sport?.bg, color: sport?.color }}>
            ...
            <div className="flex gap-1.5 mb-4 overflow-x-auto">
              {DAYS.map((day, i) => (
                <button key={i} onClick={() => setSelectedDay(i)}
                  className={`flex-none px-3.5 py-2.5 rounded-lg text-center transition-all ${selectedDay === i ? "border-2 border-[#1D9E75] bg-[#E1F5EE] text-[#1D9E75]" : "border-[1.5px] border-[#f0ede6] text-gray-500"}`}>
                  <div className="text-[11px] opacity-70">{day}</div>
                  <div className="text-base font-bold">{DATES[i]}</div>
                </button>
              ))}
            </div>
            ...
            {equipment.length > 0 && (
              <div className="bg-[#E1F5EE] rounded-xl p-3.5 mb-4">
                <div className="text-sm font-semibold text-[#0F6E56] mb-1.5 flex items-center gap-1.5">
                  <Dumbbell size={14} /> Equipment Added ({equipment.length})
                </div>
                {equipment.map((e) => (
                  <div key={e.id} className="text-sm text-gray-600">{e.name} ×{e.qty} — ₹{e.pricePerHour * e.qty}/hr</div>
                ))}
              </div>
            )}
            ...
            <div className="flex gap-2.5">
              ...
              <Button fullWidth disabled={!selectedSlot} onClick={handleConfirmBooking}>
      ...
      <BookingSummaryModal
        ...
        slot={selectedSlot}
        equipment={equipment}
        totalAmount={(bookingClub?.price || 0) + equipment.reduce((s, e) => s + e.qty * e.pricePerHour, 0)}
        onConfirm={handlePay}
        loading={payLoading}
      />
    </div>
          ...
          <div key={rental.id} className="bg-white rounded-2xl border border-[#f0ede6] p-5">
            ...
            <div className="mb-3.5">
              {rental.items.map((item, i) => (
                <div key={i} className="text-sm text-gray-600 mb-1">• {item}</div>
              ))}
            </div>
            <div className="flex justify-between items-center pt-3.5 border-t border-[#f0ede6]">
              <div>
                <div className="text-xs text-gray-500">{rental.slot}</div>
                <div className="text-lg font-extrabold text-[#08060d]">₹{rental.total}</div>
              </div>
              {rental.deposit > 0 && (
                <div className="text-xs text-gray-500 text-right">
                  Deposit<br /><span className="font-bold text-[#BA7517]">₹{rental.deposit}</span>
                </div>
              )}
            </div>
          </div>
...
function RecommendedTab({ clubs, onSelect }) {
  return (
    <div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {clubs.map((club) => (
          <div key={club.id} onClick={() => onSelect(club)} className="flex-none w-80 bg-white rounded-2xl border border-[#f0ede6] overflow-hidden cursor-pointer hover:shadow-lg transition-all">
            ...
            <div className="p-4">
              <div className="flex justify-between items-center mb-1.5">
                <h3 className="text-base font-bold m-0">{club.name}</h3>
              ...
              <div className="flex justify-between items-center">
                <span className="text-lg font-extrabold">₹{club.price}<span className="text-sm text-gray-500 font-normal"> /hr</span></span>
