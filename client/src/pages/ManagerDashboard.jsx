...
export default function ManagerDashboard() {
  ...
  return (
    <div>
          ...
          <div className="bg-white rounded-2xl border border-[#f0ede6] p-5">
            ...
            {todayBookings.map((b) => (
              <div key={b.id} className="flex justify-between items-center py-3 border-b border-[#f0ede6] last:border-0">
                <div>
                  <div className="font-semibold text-sm text-[#08060d]">{b.user} · {b.sport}</div>
                  <div className="text-xs text-gray-500">{b.court} · {b.time}</div>
                </div>
          ...
          {INIT_CLUBS[0].courts.map((court) => {
            ...
            return (
              <div key={court.id} className="bg-white rounded-2xl border border-[#f0ede6] p-4.5">
                ...
                <div className="text-xs text-gray-500 mb-3.5 flex items-center gap-1">
                  <Clock size={12} /> {court.config.openTime}–{court.config.closeTime} · {court.config.slotDuration}min slots
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" fullWidth onClick={() => setConfigCourt(court)}>
                    <Settings size={14} /> Configure
                  </Button>
                  <Button size="sm" variant="ghost" fullWidth onClick={() => setBlockCourt(court)}>
                    <Ban size={14} /> Block
                  </Button>
                </div>
              </div>
      ...
      {tab === "equipment" && (
        <InventoryTable items={inventory} onAdd={() => {}} onRestock={handleRestock} onDelete={() => {}} />
      )}
          ...
          {RENTAL_ORDERS.map((rental) => (
            <div key={rental.id} className="bg-white rounded-2xl border border-[#f0ede6] p-5">
              ...
              <div className="mb-3.5">
                <div className="text-xs text-gray-500 mb-1">Customer: <span className="font-semibold text-[#08060d]">{rental.user}</span></div>
                {rental.items.map((item, i) => (
                  <div key={i} className="text-sm text-gray-600 mb-0.5">• {item}</div>
                ))}
              </div>
              <div className="flex justify-between items-center pt-3.5 border-t border-[#f0ede6]">
                ...
                {rental.status === "active" && (
                  <Button size="sm" variant="outline" onClick={() => setReturnRental(rental)}>Return</Button>
                )}
              </div>
            </div>
          ))}
